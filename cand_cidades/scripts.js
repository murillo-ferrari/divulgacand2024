async function loadStates() {
    try {
        ...
        const response = await fetch('../busca_candidatos/resources/data/municipios-2024.json');
...
async function fetchCandidates(selectedState, cities) {
    try {
        ...
        for (const city of cities) {
            ...
            const candidateCount = candidatesData.candidatos ? candidatesData.candidatos.length : 0;
            ...
            paragraph.textContent = `${cityName}: ${candidateCount.toLocaleString('pt-BR', { minimumIntegerDigits: 1 })} candidatos`;
